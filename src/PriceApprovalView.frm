...
Private Sub InitilizeViewExtended(ByVal context As PriceApprovalView)
    ...
    With context.ViewExtended
        ...
        .HydrateComboBox context.cmbCurrency, PriceApprovalDataResources.arrListofCurrencies
        .HydrateComboBox context.cmbUnitOfMeasure, PriceApprovalDataResources.arrListOfUnitOfMeasure
    ...
    If Not Resizer.IsViewResizable(Me, viewMinimized) Then
        If Not viewMinimized Then
            MsgBox "Minimum View Size Reached." & VBA.vbNewLine & "This is the default minimum size.", _
                vbInformation, PriceApprovalSignature
        End If
    End If
...
Private Sub txtValidFrom_Enter()
    ...
    If Not Me.lblActiveUserType.Caption = "APPROVER" And _
...
Private Sub lstUsers_DblClick(ByVal Cancel As MSForms.ReturnBoolean)
    With Me.lstUsers
        If .ListIndex > 0 Then
            If .List(.ListIndex, UsersTableFields.COL_userId - 1) < 101 Then
...
Public Sub UserWantsToOpenPasswordManagerFrame(ByVal PasswordManagerFormModel As PasswordManagerModel)
    ...
    Dim uType As String: uType = LoginModel.userType
    If uType = USERTYPE_CLIENT Then
        Call this.ViewExtended.ActivateFrames(Me.frameClient, Me.framePasswordManager)
    ElseIf uType = USERTYPE_APPROVER Then
        Call this.ViewExtended.ActivateFrames(Me.frameApprover, Me.framePasswordManager, uType)
    End If
...
Public Sub AfterChangePasswordOperation()
    MsgBox "Password has been changed successfully! Please Sign-In again.", vbInformation, PriceApprovalSignature
...
Public Sub AfterUserManagerCRUDOperation(ByVal TypeOfOperation As CRUDOperations, ByVal IsSucceessfullOperation As Boolean)
    ...
    Case CRUDOperations.CRUD_OPERATION_ADDNEW
        MsgBox "New USER added successfully!", vbInformation, PriceApprovalSignature
    Case CRUDOperations.CRUD_OPERATION_UPDATE
        If IsSucceessfullOperation Then
            MsgBox "User's record has been UPDATED successfully!", vbInformation, PriceApprovalSignature
        End If
    Case CRUDOperations.CRUD_OPERATION_DELETE
        If IsSucceessfullOperation Then
            MsgBox "User has been DELETED successfully!", vbInformation, PriceApprovalSignature
        End If
    End Select
    'Refresh Data Again
    RaiseEvent OpenUserManagerFrame
...
Public Sub AfterPriceFormCRUDOperation(ByVal TypeOfOperation As CRUDOperations, ByVal IsSuccessfullOperation As Boolean)
    ...
    Case CRUDOperations.CRUD_OPERATION_ADDNEW
        If IsSuccessfullOperation Then
            MsgBox "New Record added successfully!", vbInformation, PriceApprovalSignature
            'reset price form frame
            RaiseEvent OpenPriceFormFrame
        End If
    Case CRUDOperations.CRUD_OPERATION_UPDATE
        If IsSuccessfullOperation Then
            MsgBox "Record has been UPDATED successfully!", vbInformation, PriceApprovalSignature
            'open list for based on user type
            RaiseEvent OpenDataFormFrame(DataModel.ActiveDataContainer)
        End If
    Case CRUDOperations.CRUD_OPERATION_DELETE
        If IsSuccessfullOperation Then
            MsgBox "Record has been DELETED successfully!", vbInformation, PriceApprovalSignature
            'open list for based on user type
            RaiseEvent OpenDataFormFrame(DataModel.ActiveDataContainer)
        End If
    Case CRUDOperations.CRUD_OPERATION_APPROVE
        If IsSuccessfullOperation Then
            MsgBox "Record APPROVED successfully!", vbInformation, PriceApprovalSignature
    ...
    Case CRUDOperations.CRUD_OPERATION_REJECT
        If IsSuccessfullOperation Then
            MsgBox "Record REJECTED!", vbInformation, PriceApprovalSignature
...
Private Sub UpdateWelcomeFrame(Optional FrameIdentifier As ApplicationForms = 0)
    If FrameIdentifier = FORM_LOGIN Then
        ...
        With MultiFrameViewExtended
            Call .ChangeControlProperties(Me.lblWelcomeMessage, VBA.Strings.UCase$(MESSAGE_WELCOMESCREEN_LOGOUT_STATE), &H8000000D)
    ...
    Else
        ...
        displayText = VBA.Strings.UCase$(MESSAGE_WELCOMESCREEN_LOGIN_STATE & " " & Me.lblActiveUsername.Caption)
        Call this.ViewExtended.ChangeControlProperties(Me.lblWelcomeMessage, displayText, &H8000000D)
    End If
End Sub
